...
from idaes.models.unit_models import Mixer, MomentumMixingType, Pump, HeatExchangerNTU
...
class MEAAbsorberFlowsheetData(FlowsheetBlockData):
    ...
    def _add_units(self):
        ...
        # TODO: Use basic NTU exchanger due to issues with pressure drop in PHE unit
        self.lean_rich_heat_exchanger = HeatExchangerNTU(
            hot_side={
    ...
    def _set_absorber_design_inputs(self):
        ...
        # Fix HX unit geometry
        # TODO: Values are somewhat arbitrary for now - based on values from smaller scale PHE model
        self.lean_rich_heat_exchanger.area.fix(4000)
        self.lean_rich_heat_exchanger.heat_transfer_coefficient.fix(1000)
        self.lean_rich_heat_exchanger.effectiveness.fix(0.7)
            ...
            for x in column.liquid_phase.length_domain:
                ...
                if self.config.surrogate_enhancement_factor_model is None:
                    ...
                    if x != column.liquid_phase.length_domain.last():
                        ...
                        # ssf(column.omega[t, x], 1e-4)
                        # cst(column.enhancement_factor_eqn1[t, x], 1e-4)
                        # cst(column.enhancement_factor_eqn2[t, x], 1e-4)
    ...
    def strip_statevar_bounds_for_absorber_initialization(self):
        ...
        for x in self.absorber.liquid_phase.length_domain:
            for j in self.absorber.config.liquid_phase.property_package.true_phase_component_set:
                self.absorber.liquid_phase.properties[0, x].flow_mol_phase_comp_true[j].setlb(None)
                self.absorber.liquid_phase.properties[0, x].flow_mol_phase_comp_true[j].domain = pyo.Reals

                self.absorber.liquid_phase.properties[0, x].mole_frac_phase_comp_true[j].setlb(None)

            for j in self.absorber.config.liquid_phase.property_package.apparent_species_set:
                self.absorber.liquid_phase.properties[0, x].mole_frac_comp[j].setlb(None)

            for j in self.absorber.config.liquid_phase.property_package.apparent_phase_component_set:
                self.absorber.liquid_phase.properties[0, x].mole_frac_phase_comp.setlb(None)

        for x in self.absorber.vapor_phase.length_domain:
            for j in self.absorber.config.vapor_phase.property_package.component_list:
                self.absorber.vapor_phase.properties[0, x].mole_frac_comp[j].setlb(None)

            for j in self.absorber.config.vapor_phase.property_package._phase_component_set:
                self.absorber.vapor_phase.properties[0, x].mole_frac_phase_comp[j].setlb(None)
